...
import { GET_API_OPTIONS, TMDB_BASE_URL } from "./const";
import {
	findMovieByIdRequestSchema,
	findPopularMoviesRequestSchema,
	searchSchema,
} from "./schema/req";
import type { FindTrendingMoviesResponse } from "./schema/res";
...
export const fetchTrendingMovies = createServerFn({ method: "GET" }).handler(
	async () => {
		...
		return response.json() as Promise<FindTrendingMoviesResponse>;
	},
);
